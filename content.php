<?php
	...
	$hasil_variabel	=mysql_query($sql_variabel);
	$row_variabel	=mysql_fetch_assoc($hasil_variabel);
...
if ($_GET['aksi']==himpunan) {   
	if ($_GET['di']==simpan) {  
	$id=$_POST['id'];
	mysql_query(" update tbl_himpunan SET  
	...
	if ($_GET['di']==edit) {   
	  $sethf=mysql_query("SELECT * FROM tbl_himpunan where id='$_GET[ids]'");
	  while ($setrhf=mysql_fetch_array($sethf))
...
echo "<h2>Setting</h2>";
  $seth=mysql_query("SELECT * FROM tbl_himpunan where kelompok='$_GET[id]'");
  $noh=0;
  echo "<table border=1> <tr bgcolor=#9DC5FF> <td>No. </td> <td>Himpunan Fuzzy</td> <td>Domain</td> <td>Aksi</td></tr>";
  while ($setrh=mysql_fetch_array($seth))
  { $noh++;
